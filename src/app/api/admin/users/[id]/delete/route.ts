...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!eligibility.ok) {
    ...
    // Note also: when bookings block, `pendingPayouts: 0`/`owedAmount: 0` may
    // mean "unknown" rather than "none", since none of the three queries is
    // short-circuited.
    if (
      eligibility.blocking.bookings.length > 0 ||
      eligibility.blocking.pendingPayouts > 0 ||
      eligibility.blocking.owedAmount > 0
    ) {
