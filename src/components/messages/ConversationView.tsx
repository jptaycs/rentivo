...
export function ConversationView({ header, messages, currentUserId, onSend, onBack }: ConversationViewProps) {
  ...
  const { signed: signedImageUrls, failed: failedImagePaths } = useMessageImageUrls(
    messages.map((m) => m.image_url)
  )
        ...
        {messages.length === 0 ? (
          ...
          <>
            ...
            {messages.map((msg) => {
              ...
              const imageFailed = hasImage && !imageUrl && Boolean(failedImagePaths[msg.image_url as string])
              return (
                <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                  <div className="max-w-[75%] group">
                    <div className={`rounded-2xl text-sm leading-relaxed overflow-hidden ${
                      ...
                      {hasImage && !imageUrl && !imageFailed && (
                        <div className="h-40 w-56 max-w-full bg-gray-100 animate-pulse" aria-label="Loading photo" />
                      )}
                      {imageFailed && (
                        <div
                          className={`h-24 w-56 max-w-full flex items-center justify-center text-xs ${
                            isMe ? 'bg-white/10 text-blue-100' : 'bg-gray-50 text-gray-400'
                          }`}
                        >
                          Photo unavailable
                        </div>
