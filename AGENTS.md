...
- [ ] Notification preferences: wire the persisted toggles to actually gate sending (`notify_new_booking` → host new-booking email, `notify_messages` → `notifyNewMessage`); `reminders`/`promos` have no sender to gate yet
