...
- [x] **Three test listings from earlier sessions are live on the production marketplace —
  DECIDED 2026-09-04: leave them.** The owner was shown the finding and chose to keep them
  visible. Do not deactivate, hide or delete them, and do not raise this again as a cleanup
  task. Detail kept below because the fixtures matter.
    *Original finding:* **Three test listings from earlier sessions are live on the production marketplace**
  (found 2026-09-04 while chasing a console error on `/search`). All owned by the seed host
  `a0000000-…`, all with no photos — which is how they surfaced, via the missing-placeholder
  400 fixed the same day. **Two are `is_active` and not draft, so real visitors see them:**
  "PayMongo Test Lens (instant)" (3 bookings — one `confirmed`/`paid`, one `pending`/`unpaid`,
  one `cancelled`/`refunded`) and "Refund Test Lens A" (1 booking, `completed`/`paid`). The
  third, "Canon EOS 90D (booking e2e)", is already inactive.
  **Deliberately not touched.** They are not junk: `AGENTS.md` references them by name, the
  demo host's only `paid` payout request is built on one of their bookings, and the sole
  refunded booking in the database (`RNT-75715D`) lives on one — so deactivating or deleting
  them would invalidate the fixtures several documented verifications depend on, and the
  `confirmed`/`paid` booking is somebody's live rental record. Deactivating (never deleting,
  per the `924ca665` precedent) hides them from the storefront without touching bookings, but
  that is a marketplace decision for the owner, not a cleanup an agent should make.
