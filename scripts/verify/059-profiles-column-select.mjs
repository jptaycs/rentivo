...
// for setup/cleanup. The demo host gets a temporary probe notify_promos value
// (059's private-column list — qr_payment_label no longer exists, dropped by
// migration 072 along with qr_payment_url when host-QR was retired) so the
// leak is tested against a real non-default value, restored afterwards.
...
const PROFILE_COLUMNS = 'id, full_name, avatar_url, is_verified, is_host, host_rating, host_review_count, response_time_hours, bio, city, created_at'
...
// Setup: give the demo host a probe notify_promos value (default false) so a
// leak would return a real, recognizably-non-default value.
const { body: [orig] } = await admin(`profiles?select=notify_promos,bio&id=eq.${hostId}`)
await admin(`profiles?id=eq.${hostId}`, { method: 'PATCH', body: JSON.stringify({ notify_promos: true }) })

try {
  // ── Private columns: denied for anon and for a signed-in stranger ──
  for (const [who, tok] of [['anon', null], ['renter session', renterTok]]) {
    for (const col of ['notify_promos', 'notify_messages', 'updated_at']) {
    ...
    // The private column must not appear anywhere in a permitted response either.
    ...
    check(`${who}: probe column absent from the permitted response`, !('notify_promos' in (pub.body?.[0] ?? {})))
  }

  // ── Even the OWNER cannot read the private columns through the table ──
  const ownStar = await asUser(hostTok, `profiles?select=notify_promos&id=eq.${hostId}`)
  check('owner: table read of own notify_promos denied (column grant, not RLS)', denied(ownStar), `${ownStar.status}`)
  ...
  check('host: own row includes the private probe value', mine.body?.notify_promos === true)
  ...
  const wPromos = await asUser(hostTok, `profiles?id=eq.${hostId}`, { method: 'PATCH', headers: { Prefer: 'return=minimal' }, body: JSON.stringify({ notify_promos: false }) })
  check('host: own notify_promos update still works (025 grant intact)', wPromos.status === 204, `${wPromos.status}`)
  ...
  await admin(`profiles?id=eq.${hostId}`, { method: 'PATCH', body: JSON.stringify({ notify_promos: orig.notify_promos, bio: orig.bio }) })
  const { body: [restored] } = await admin(`profiles?select=notify_promos,bio&id=eq.${hostId}`)
  check('cleanup: demo host notify_promos + bio restored', restored.notify_promos === orig.notify_promos && restored.bio === orig.bio)
}
console.log(fails ? `\n${fails} FAILED` : '\nALL PASSED')
